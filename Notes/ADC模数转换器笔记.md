...
实验目标: **实现ADC多通道转换，使用PA0-PA3作为输入通道，显示每个通道的ADC值**

### ADC多通道转换初始化

在`adc.h`文件中，配置ADC1为单通道转换模式，使用PA0-PA3作为输入通道。ADC时钟为PCLK2/6，扫描模式和连续转换模式被禁用。
...
在`adc.c`文件中，获取ADC多通道转换值的函数主要有以下几个步骤：

1. **配置规则组通道**：使用`ADC_RegularChannelConfig`函数配置ADC1的规则组第1个位置为指定通道N，采样时间为55.5个时钟周期。
2. **使能软件触发转换**：使用`ADC_SoftwareStartConvCmd`函数使能ADC1的软件触发转换。
3. **等待转换完成**：使用`ADC_GetFlagStatus`函数检查转换完成标志位`ADC_FLAG_EOC`，直到转换完成。
4. **获取转换结果**：使用`ADC_GetConversionValue`函数获取ADC1的转换结果。
...
在`main.c`文件中，ADC多通道转换实验的主要流程如下：

1. **模块初始化**：调用`OLED_Init()`和`ADC1_Init()`函数初始化OLED显示屏和ADC模块。
2. **OLED显示静态文本**：使用`OLED_ShowString`函数在OLED上显示每个通道的ADC值。
3. **主循环获取ADC值并显示**：在死循环中调用`ADC_GetValue()`函数获取ADC转换结果，并使用`OLED_ShowNum`函数在OLED上显示每个通道的ADC值。
